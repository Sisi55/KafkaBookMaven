...
public class KafkaEnrichmentSample {
    ...
    public static void main(String[] args) throws Exception {
        ...
        KStream<String, String> mqttSource = builder.stream(
                sensordataTopicName,
                Consumed.with(Serdes.String(), Serdes.String())
        );

        GlobalKTable<String, String> deviceMasterTable = builder.globalTable(
                masterdataTopicName,
                Consumed.with(Serdes.String(), Serdes.String())
        );

        mqttSource.map((key, value) -> KeyValue.pair(value.split(",")[0], value))
