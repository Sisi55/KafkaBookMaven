...
public class Streaming2 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        KStream<String, JsonNode> metrics = builder.stream("kafka.metrics",
        ...
        metrics.filter((host, root) -> root.has("value") && root.has("hostname") && root.has("timestamp"))
                ...
                .aggregate(
                        ...
                        (k,v,agg) -> {
                            ...
                            if(!agg.has("timestamp_0"))
                                o.put("timestamp_0", v.get("timestamp").asLong());
                            if(!agg.has("first"))
                                o.put("fist", v.get("BytesIn").asLong());

                            o.put("last", v.get("BytesIn").asLong());
                            o.put("hostname", k);
                            o.put("timestamp", v.get("timestamp").asLong());
                            o.put("width",
                                  o.get("timestamp").asLong() - o.get("timestamp_0").asLong());
                            return o;
                        },
